...
値が真偽の評価をされる場面は、if文 `if (xxx) { }`、三項演算子 `xxx ? 'a' :'b'`、while文 `while (xxx) { }` などです。
より正確には、値がスカラーコンテキストで真偽評価されています。スカラーコンテキストについてはFIXME章で解説します。
...
詳しくは、ドキュメントを参照してください。https://perldoc.jp/docs/perl/5.42.0/perlop.pod#Here-docs
...
そこで、Perlでは、参照カウント方式の自動ガベージコレクション付きのポインターのリファレンスを導入します。
