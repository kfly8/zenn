...
Perl v5.26以降では、`xx`修飾子を利用できます。`xx`修飾子を使うと、正規表現内の空白文字が無視され、コメント（`#`以降）も書けるようになります。さらに、ブラケット文字クラス内（`[]`の中）でも、バックスラッシュでエスケープされていないスペースやタブが無視されるため、複雑な正規表現を読みやすく書くことができます。
...
`xx`修飾子を使用する場合、実際の空白文字をマッチさせたいときは、`\ `（バックスラッシュ+スペース）または`\s`を使用します。
...
複数のファイルに対して一括で正規表現置換を行うには、`perl`コマンドラインと`-i`オプション（in-place編集）、`-p`オプション（各行を処理）、`-e`オプション（スクリプトを指定）を組み合わせます。

```bash
# テストファイル内のすべての is を is_deeply に置換
perl -i -pe 's/\bis\b/is_deeply/g' t/**/*.t

# バックアップを作成しながら置換（.bakファイルが作成される）
perl -i.bak -pe 's/old_function/new_function/g' lib/**/*.pm

# 特定のパターンを含む行のみ置換
perl -i -pe 's/FIXME章/16章/g if /正規表現/' books/**/*.md
```

この方法は、リファクタリングや一括修正を行う際に便利です。
...
sub mask($text) {
    return $text =~ s/(Password: )\w+/$1****/gr;
}

my $secret = "secret123";
say mask("User: admin, Password: $secret, Status: OK");
# "User: admin, Password: ****, Status: OK"
```
