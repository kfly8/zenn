...
今回は、304 Not Modifiedで返すようにする調整だけしました。
具体的には、hono/vite-build/bun だと、静的ファイルの配信が304 Not Modifiedで配信する方法がわからなかったので、代わりにhono/vite-build を利用して、自前で静的ファイルのルーティングを行っています。
まだ調整の余地がありそうです。
